...
// 引入生成音频的函数uploadImage 太长了 ->gennerateAudio 模块化 -->复用-->lib/audio.js
import {generateAudio} from './service/audio';
import {getReplyData} from './service/getreplydata';
import './App.css'
...
function App() {
  ...
  const [explainations, setExplaination] = useState([]);
  ...
  const uploadImage = async (imgData) => {
    ...
    //   })
    // })
    // const data =getReplyData(imgData);
    // console.log(data);
    const replyData =  await getReplyData(imgData);
    setWord(replyData.representative_word)
    console.log(replyData.
      representative_word
      );
    setSentence(replyData.example_sentence);
    // const audioUrl = await gennerateAudio(replyData.example_sentence);
